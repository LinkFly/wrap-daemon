...
void printToSyslog(const string& str) {
    std::string msg;
    msg += std::string("[") + std::to_string(getpid()) + std::string("] ");
    msg += str;
    syslog(9, msg.c_str());
}
...
int main(int argc, char** argv)
{
    ...
    */
    ...
    pid = fork();
    /*
    printToSyslog("after-fork");
    std::cout << "cur pid = " << getpid();
    std::cout << "ppid = " << (int)getppid() << std::endl;
    */
    ...
    else if (getpid() != curpid) // если это потомок
    {
        printToSyslog("Yes!! This is child!");
        ...
        std::cout << "(" << (int)getpid() << "): child-cout" << std::endl;
        std::string strMsg = std::string("(") + std::to_string(getpid()) + "): child-cerr\n";
        //printToSyslog(strMsg.c_str());
